      ...
      <>
        <div className={`relative ${className}`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-all ${
              isDragging
                ...
                : 'bg-teal-600/20 border-teal-500/40 text-teal-500 hover:bg-teal-500/15 hover:border-teal-500 hover:text-teal-300 hover:shadow-lg hover:shadow-teal-500/10'
            } border`}
          >
          ...
          {analysisResults.length > 0 && !showSuccess && (
            <div className="absolute top-full left-0 mt-2 px-3 py-1.5 bg-teal-500/10 border border-teal-500/15 rounded-lg text-xs text-teal-500 whitespace-nowrap z-50">
        ...
        >
          <div className="flex flex-col items-center gap-3">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
              isDragging ? 'bg-teal-500/10' : 'bg-[var(--bg-secondary)]'
            }`}>
              <svg className={`w-6 h-6 ${isDragging ? 'text-teal-500' : 'text-[var(--text-muted)]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
