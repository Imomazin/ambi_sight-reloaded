      ...
      <>
        <div className={`relative ${className}`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-all ${
              isDragging
                ...
                : 'bg-purple-600/20 border-purple-500/40 text-purple-400 hover:bg-purple-500/30 hover:border-purple-400 hover:text-purple-300 hover:shadow-lg hover:shadow-purple-500/10'
            } border`}
          >
          ...
          {analysisResults.length > 0 && !showSuccess && (
            <div className="absolute top-full left-0 mt-2 px-3 py-1.5 bg-purple-500/20 border border-purple-500/30 rounded-lg text-xs text-purple-400 whitespace-nowrap z-50">
        ...
        <div
          ...
          className={`relative cursor-pointer border-2 border-dashed rounded-xl p-6 text-center transition-all ${
            isDragging
              ...
              : 'border-[var(--border-color)] hover:border-purple-500/50 hover:bg-[var(--bg-card-hover)]'
          }`}
        >
          <div className="flex flex-col items-center gap-3">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
              isDragging ? 'bg-purple-500/20' : 'bg-[var(--bg-secondary)]'
            }`}>
              <svg className={`w-6 h-6 ${isDragging ? 'text-purple-400' : 'text-[var(--text-muted)]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
