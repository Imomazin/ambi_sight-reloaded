  ...
  if (!tool) {
    return (
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 text-center">
        ...
        <button
          onClick={onClose}
          className="px-4 py-2 bg-teal-500 hover:bg-teal-500 text-white rounded-lg transition-colors"
        >
          Back to Dashboard
        </button>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-4 md:p-6">
        ...
        <div className="flex items-start gap-3 md:gap-4">
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-gradient-to-br from-teal-500/10 to-purple-500/20 flex items-center justify-center text-2xl md:text-3xl flex-shrink-0">
            {tool.icon}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6">
            <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4 flex items-center gap-2">
              <span className="text-teal-500">👥</span> Best For
            </h2>
            <ul className="space-y-3">
              {tool.bestFor.map((item, idx) => (
                <li key={idx} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-teal-500/10 text-teal-500 flex items-center justify-center text-xs">
                    ✓
                  </span>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6">
            <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4 flex items-center gap-2">
              <span className="text-purple-500">❓</span> Key Questions to Answer
            </h2>
            <div className="space-y-4">
              {tool.keyQuestions.map((question, idx) => (
                <div key={idx} className="flex items-start gap-4 p-4 bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-color)]">
                  <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500/20 to-teal-500/10 flex items-center justify-center text-sm font-bold text-[var(--text-primary)]">
                    {idx + 1}
                  </div>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6">
            ...
            <div className="bg-[var(--bg-secondary)] rounded-xl p-8 border-2 border-dashed border-[var(--border-color)] text-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <button className="w-full sm:w-auto px-5 py-3 bg-teal-500 hover:bg-teal-500 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2 touch-manipulation">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6">
            ...
            <div className="space-y-2">
              <button className="w-full flex items-center gap-3 p-3 bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-color)] text-sm text-[var(--text-secondary)] hover:border-teal-500/15 hover:text-teal-500 transition-colors">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Need Help */}
          <div className="bg-gradient-to-br from-teal-500/10 to-purple-500/10 border border-teal-500/10 rounded-xl p-6">
            ...
            <Link
              href="/pricing"
              className="inline-flex items-center gap-2 text-sm text-teal-500 hover:text-teal-300 font-medium"
            >
              Book Consultation
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
