...
type OAuthStep = 'select' | 'verify' | 'complete';
...
export default function SignInPage() {
  ...
  const [oauthStep, setOauthStep] = useState<OAuthStep>(providerParam ? 'verify' : 'select');
  ...
  const handleOAuthVerify = (e: React.FormEvent) => {
    ...
    const provider = authMethod as 'google' | 'microsoft';
    ...
    const isValidDomain = validDomains[provider]?.some(d => emailDomain === d || emailDomain?.endsWith('.' + d));

    // For demo purposes, also accept any email
    if (!emailDomain) {
    ...
    setTimeout(() => {
      const providerNames = {
        google: oauthEmail.split('@')[0],
        microsoft: oauthEmail.split('@')[0],
      };
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          {/* OAuth Verification Mode */}
          {oauthStep === 'verify' && (authMethod === 'google' || authMethod === 'microsoft') ? (
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6">
              <div className="text-center mb-6">
                ...
                <h1 className="text-2xl font-bold text-[var(--text-primary)] mb-2">
                  Verify Your {authMethod === 'google' ? 'Gmail' : 'Outlook'} Account
                </h1>
                <p className="text-[var(--text-muted)] text-sm">
                  Enter your {authMethod === 'google' ? 'Gmail' : 'Outlook'} email to {mode === 'signin' ? 'sign in' : 'create your account'}
                </p>
              </div>
              ...
              <form onSubmit={handleOAuthVerify} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">
                    {authMethod === 'google' ? 'Gmail' : 'Outlook'} Email Address
                  </label>
                  <input
                    type="email"
                    value={oauthEmail}
                    onChange={(e) => setOauthEmail(e.target.value)}
                    placeholder={authMethod === 'google' ? 'you@gmail.com' : 'you@outlook.com'}
